package  org.mig.model.vo.media
{
	...
	public class MediaCategoryNode extends ContentNode {
		...
		[Bindable] public var numFolders:int = 0;
		[Bindable] public var numItems:int = 0;
		
		private var diskFiles:Array;
		...
		override protected function retrieve():void {
			...
			params.mapping = config.@filedir + "/" + directory;
			
			var command:RetrieveContentCommand = new RetrieveContentCommand(this,params,MediaData,handleChildren);
			command.execute();
		...
		private function handleChildren(results:Array,token:AsyncToken=null):void {
			...
			for each (var item:MediaData in results) {
				if(item.type.toString() == "folder") {
					...
					if(directory != "" && directory != null)
						newdirectory = directory + "/" + item.name.toString();
					else
						newdirectory = item.name.toString();
					var categoryNode:MediaCategoryNode = new MediaCategoryNode(item.name, _config, item, this, newdirectory,this.privileges);
					children.addItem(categoryNode);
					numFolders += 1;					
				}
				else
					diskFiles.push(item);
		...
		private function fetchChildrenFromDB():void {
			var params:Object = new Object();
			...
			var command:RetrieveContentCommand = new RetrieveContentCommand(this,params,MediaData,handleDBComplete);
			command.execute();
		}
